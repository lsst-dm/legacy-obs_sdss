...
class SelectFluxMagInfoConfig(SelectSdssImagesConfig):
    table = pexConfig.Field(
        doc = "Name of database table",
        dtype = str,
    ...
    database =  pexConfig.Field(
        ...
        dtype = str,
        default = "krughoff_early_prod_11032012",
        )
...
class SelectSdssfluxMag0Task(BaseSelectImagesTask):
    """Select SDSS images suitable for coaddition
    """
    ConfigClass = SelectFluxMagInfoConfig

    @pipeBase.timeMethod
    def run(self, coordList, filter, run):
        """Select flugMag0's of SDSS images in a particular region
        ...
        self.config.database = "krughoff_early_prod_11032012"
...
where ccdExp.scienceCcdExposureId = idList.scienceCcdExposureId and """ % (self.config.table,)
        else:
            # no region specified; look over the whole sky
            queryStr += """
from %s as ccdExp where """ % (self.config.table,)
        ...
        self.log.info("Found %d exposures runs" % \
            (len(exposureInfoList)))
